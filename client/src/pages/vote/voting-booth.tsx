...
export default function VotingBooth() {
  ...
  usePageTitle(campaign?.title ? `Votar: ${campaign.title}` : "Votación");
  ...
  const now = new Date();
  const isTimeValid = campaign
    ? new Date(campaign.startDate) <= now && new Date(campaign.endDate) >= now
    : false;
      ...
      <PublicLayout>
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-12 text-center max-w-xl mx-auto border border-slate-200 dark:border-slate-800 shadow-sm">
          ...
          <p className="text-slate-600 dark:text-slate-400">
            El enlace de votación que siguió no es válido o ha caducado. Por
            favor, consulte con el administrador de la elección.
          </p>
        </div>
      </PublicLayout>
    );
      ...
      <PublicLayout>
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-12 text-center max-w-xl mx-auto border border-slate-200 dark:border-slate-800 shadow-sm">
          ...
          <p className="text-slate-600 dark:text-slate-400 text-lg mb-8 leading-relaxed">
            Su participación ha sido procesada correctamente.{" "}
          ...
          <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800 flex items-center gap-3 justify-center text-slate-500 dark:text-slate-400">
            ...
            <span className="text-xs font-bold uppercase tracking-widest">
              Hash de Voto Generado de forma Segura
            </span>
          </div>
        </div>
      </PublicLayout>
          ...
          <div className="mt-8 inline-flex items-center gap-3 px-6 py-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/50 text-amber-800 dark:text-amber-400 rounded-2xl">
            ...
            <div className="text-left">
              ...
              <p className="text-sm opacity-90">
                Por favor, vuelva a consultar durante el período de votación
                designado.
              </p>
            </div>
          </div>
        ...
        <div className="w-full">
          ...
          <div className="mb-8">
            <h3 className="font-display font-bold text-xl text-slate-900 mb-6 flex items-center gap-2">
              <span className="bg-primary text-white w-8 h-8 rounded-full flex items-center justify-center text-sm shadow-md">
                1
              </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
              {loadingCandidates
                ? [1, 2].map((i) => (
                    <div
                      key={i}
                      className="h-64 bg-white dark:bg-slate-900 rounded-3xl animate-pulse border border-slate-100 dark:border-slate-800"
                    />
                  ))
                : candidates?.map((candidate) => (
                    <div
                      key={candidate.id}
                      onClick={() =>
                        canVote && setSelectedCandidate(candidate.id)
                      }
                      className={`
              ...
              >
                ...
                >
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <h4 className="font-display font-bold text-xl text-slate-900 dark:text-white italic">
                      Confirmar Identidad
                    </h4>
                  </div>
                  ...
                  <form onSubmit={handleVoteSubmit} className="space-y-6">
                    <div>
                      <label className="block text-xs font-black uppercase text-slate-400 dark:text-slate-500 tracking-widest mb-3 ml-1">
                    ...
                    {voteError && (
                      <div className="bg-rose-50 dark:bg-rose-900/30 border border-rose-100 dark:border-rose-900/50 rounded-xl p-4 flex items-center gap-3 text-rose-700 dark:text-rose-400 text-sm">
                        ...
                        <p>
                          {(voteError as any).message ||
                            "Algo salió mal. Verifique su ID."}
                        </p>
                      </div>
                    )}
                    <button
                  ...
                  <p className="text-center text-xs text-slate-400 mt-6 flex items-center justify-center gap-1.5">
                    <ShieldCheck className="w-4 h-4" /> Este voto es definitivo
                    y está vinculado de forma segura a su ID.
                  </p>
                </motion.div>
              </motion.div>
            )}
